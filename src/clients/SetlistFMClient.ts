    ...
    async getSetlistsByArtistName(artistName: String, numberOfSets: number) {
        ...
        try {
            ...
            for (let x = 0; x < sets.length; x++) {
                ...
                songs.push(sets[x].set.map(i => i.song).flat().map(x => x.name.trim()))
            }

            // Return only sets that have songs
            const setsWithSongs = songs.filter((subArray: any[]) => subArray.length > 0)

            const numberOfShowsToReturn = (numberOfSets < setsWithSongs.length ? numberOfSets : setsWithSongs.length)
