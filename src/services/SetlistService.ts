...
export class SetlistService {
  ...
  async getAverageSetlistByArtistName(
    ...
    const listOfSetlists = await this.getSetlistsByArtistAndNumberOfShows(
      artistName,
      numberOfSets,
    ).then((response) => {
      return response;
    ...
    const averageSetListLength: number = Math.round(
      setlistLength / listOfSetlists.length,
    );

    const averageSetlist: AverageSetlist = {
      artistName,
      averageSetLength: averageSetListLength,
      songs: this.compileSongStats(listOfSetlists, averageSetListLength).map(
        (i) => i.title,
      ),
    };
